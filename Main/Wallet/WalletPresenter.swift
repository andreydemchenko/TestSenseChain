...
class WalletPresenter {
    ...
    private var data: WalletBalanceRes?
    ...
    private func prepareData() {
        if let data {
            let chechBalance = SectionWallet(data: [WalletModelCell(image: UIImage(named: "cheking_account_ico")!, text: "Checking account", balance: data.checking_balance?.toDouble)])
            let savBalance = SectionWallet(data: [WalletModelCell(image: UIImage(named: "saving_account_ico")!, text: "Saving account", balance: data.loan_balance?.toDouble)])
            let loanBalance = SectionWallet(data: [WalletModelCell(image: UIImage(named: "loan_account_ico")!, text: "Loan account", balance: data.loan_balance?.toDouble)])
            let grantedBalance = SectionWallet(data: [WalletModelCell(image: UIImage(named: "granted_account_ico")!, text: "Granted loan account", balance: data.granted_loan_balance?.toDouble)])
            let frozenBalance = SectionWallet(data: [WalletModelCell(image: UIImage(named: "frozen_account_ico")!, text: "Frozen account", balance: data.frozen_balance?.toDouble)])
